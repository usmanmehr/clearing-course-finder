...
// The student's own A-level letter profile (e.g. "AAB") - derived ONLY from the
// grades they entered, so it is exact (not an estimate). Returns null for
// BTEC/mixed entries, where a single A-level-style letter profile would not be
// accurate; in that case the frontend shows the points figure alone.
function aLevelProfile(subjects) {
  const V = { 'A*': 56, A: 48, B: 40, C: 32, D: 24, E: 16 };
  const letters = [];
  for (const s of subjects || []) {
    const type = s.type && s.type !== 'alevel' ? s.type : 'alevel';
    if (type !== 'alevel') return null;
    const g = (s.grade || '').toUpperCase();
    if (!(g in V)) return null;
    letters.push(g);
  }
  if (letters.length < 2) return null;
  letters.sort((a, b) => V[b] - V[a]);
...
function statusBadge(clearingStatus) {
  ...
  if (s.includes('closed')) return { colour: 'Red', label: 'Not in Clearing at our last update - confirm with the university' };
  if (s.includes('opens')) return { colour: 'Amber', label: 'Expected to open on Results Day - not yet confirmed' };
  return { colour: 'Amber', label: 'Listed in Clearing at our last update - confirm with the university' };
}
...
export const handler = async (event) => {
  ...
  // STEP 2 - rate limiting (per IP: 30/min, 700/hour).
  ...
  const perHour = await checkRateLimit(RATE_LIMITS_TABLE, `ip#${sourceIp}#h`, 700, 3600);
  if (!perHour.allowed) {
    await putMetric('RateLimitedCount', 1);
  ...
  try {
    ...
    for (const u of CONTACTS_CACHE) {
      ...
      courses.push({
        ...
        courseTitle: resolved ? subjectName : 'Multiple subjects - see clearing page',
        ...
        // students: it is derived from institution tier, not any published
        // course requirement, so showing it as a grade risks being wrong.
        _clearingGradeNumeric: gradeNumeric,
        graduateProspects,
    ...
    let filtered = courses.filter((c) => c._clearingGradeNumeric <= candidateTotal);
    ...
    const results = ranked.slice(0, limit).map((c) => {
      const { _nationalMedianSalary, _clearingGradeNumeric, ...rest } = c;
      return rest;
    });
    ...
    return json(200, {
      results,
      salaryContext,
      userTariffPoints: candidateTotal,
      userTariffGrades: aLevelProfile(body.subjects),
      totalMatches: filtered.length,
      queryId,
