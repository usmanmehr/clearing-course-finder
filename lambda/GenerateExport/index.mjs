...
function buildXlsx(rows, salaryContext) {
  const headers = ['University', 'Course', 'Status', 'Entry requirement',
    'Graduate prospects % (CUG 2027)', 'Clearing phone', 'Clearing page'];
  const colWidths = [26, 30, 22, 40, 22, 24, 40];
  ...
  rows.forEach((row) => {
    ...
    const vals = [row.universityName, row.courseTitle, row.statusBadge?.label || '',
      'Not held - confirm on UCAS or call the university on Results Day',
...
function buildPdf(rows, salaryContext) {
  ...
  lines.push({ t: 'We do not hold course entry requirements. Confirm them and live vacancies on UCAS or by phone on Results Day.', size: 9 });
  if (salaryContext && salaryContext.nationalMedianSalary != null) {
    lines.push({
  ...
  rows.slice(0, 30).forEach((r, i) => {
    ...
    lines.push({ t: `   ${r.statusBadge?.label || ''} | Entry requirement not held - confirm on UCAS/by phone | ${prospects}`, size: 9 });
